...
public class CustomerIssueService {
	...
	public CCRepresentative getAvailableCCR()
	{
		...
		for(CCRepresentative list1: ccrList) {
			list.add(list1.getIssuesInProgress());
		}
		Collections.sort(list);
		int min= list.get(0);
		for(CCRepresentative ccr1: ccrList) {
			if(min == ccr1.getIssuesInProgress()) {
				ccrByMinIssues = ccr1;
			}
		}
	...
	public CCRepresentative getCCRById(String ccrId) {
		...
		//Type Your Code here
		for(CCRepresentative ccr: ccrList) {
			if(ccr.getCcRId() == ccrId) {
				ccRep= ccr;
			}
		}
